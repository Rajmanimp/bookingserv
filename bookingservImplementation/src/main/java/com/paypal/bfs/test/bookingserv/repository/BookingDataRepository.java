...
public interface BookingDataRepository extends JpaRepository<BookingData, Long> {
	...
	@Query(value = "SELECT * FROM booking_data u WHERE u.first_name = ?1 and u.last_name = ?2 and date_of_birth = ?3 and request_time > DATEADD('MINUTE',-1, NOW())", nativeQuery = true)
